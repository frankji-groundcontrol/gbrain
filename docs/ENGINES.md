...
alongside it. A `?search_path=<schema>,extensions,public` suffix on the URL
runs the whole brain inside a dedicated Postgres schema — useful when it
co-tenants a shared database with an app or other brains. As of v0.43
(2026-07), the explicit `postgres_schema: "groundcontrol"` config field (env
`GBRAIN_POSTGRES_SCHEMA=groundcontrol`) activates a fully restricted dedicated
mode: the `groundcontrol_app` role owns, initializes, migrates, and uses only
the `groundcontrol` schema, with fail-before-DDL preflight and post-migration
final verification. Legacy URL-suffix custom-schema installations and default-
`public` Postgres installations keep their current behavior byte-for-byte.
And `direct_database_url` (env `GBRAIN_DIRECT_DATABASE_URL`, or the
`direct_database_url` config field) overrides the DDL/bulk "direct" pool,
whose derived `db.<project-ref>.supabase.co:5432` host is IPv6-only and can
black-hole `initSchema` on IPv6-hostile networks. Both are covered in
