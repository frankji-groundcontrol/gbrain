...
**Example takes:**
- `holder=people/alice-chen kind=bet` "AI will replace 50% of coding by 2030" (w=0.75)
- `holder=people/charlie-wu kind=take` "Acme has strong retention" (w=0.80)
- `holder=world kind=fact` "WidgetCo raised $100M Series C" (w=1.0)
- `holder=brain kind=hunch` "Alice has a hero/rescuer pattern" (w=0.70)
...
**Example facts:**
- `kind=event` "I have a meeting with Alice tomorrow"
...
beliefs (an investor's assessment of a company, a peer's view on hiring, a
...
1. **Holder ≠ subject.** "Alice has a hero/rescuer pattern" → holder=brain, NOT people/alice-chen
