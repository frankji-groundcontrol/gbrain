...
|---|---|---|---|
| `bun run test` | Resource-safe unit-test loop. Defaults to one shard and one Bun worker via `scripts/run-unit-parallel.sh`, builds one validated PGLite schema snapshot with Bun's legacy 1536d test configuration, then runs normal files in batches of eight so PGLite/WASM memory is released before accumulating without restoring the snapshot per file. Genuine empty-database fixtures cold-init in isolation; the four bootstrap-contract suites are excluded from this local loop but remain in Docker CI. A serial pass follows. Set `SHARDS`, `GBRAIN_TEST_MAX_CONCURRENCY`, or `GBRAIN_TEST_FILES_PER_PROCESS` to opt into parallelism or tune the batch size. Excludes `*.slow.test.ts` and `test/e2e/*`. No pre-checks, no typecheck. | Minutes for the whole suite; run focused files while editing | Inner edit loop. Default. |
...
- **Local fast loop** (`scripts/run-unit-shard.sh` via the parallel wrapper) uses round-robin-by-index sharding and EXCLUDES `*.slow.test.ts`, `*.serial.test.ts`, and four bootstrap-contract PGLite suites. It defaults to one shard, one Bun worker, and eight files per process; PGLite/WASM memory is released between small batches without reloading the 44 MB snapshot for every file. The wrapper rebuilds one gitignored `test/fixtures/pglite-snapshot.tar` using Bun's legacy 1536d test configuration, so ordinary PGLite tests restore post-migration state instead of replaying the schema; genuine empty-database fixtures run cold in their own process. Set `SHARDS`, `GBRAIN_TEST_MAX_CONCURRENCY`, and/or `GBRAIN_TEST_FILES_PER_PROCESS` to tune it. Local trades wallclock and the four bootstrap suites for bounded resource use; CI catches what local skips.
...
- `*.test.ts` → fast loop (one shard/worker and eight files per process by default; `SHARDS`, `GBRAIN_TEST_MAX_CONCURRENCY`, and `GBRAIN_TEST_FILES_PER_PROCESS` tune it).
...
- `test/e2e/groundcontrol-dedicated-schema.test.ts` — `DATABASE_URL`-gated lifecycle E2E for the restricted `groundcontrol` dedicated schema mode (2026-07). Provisions a uniquely-named disposable DB + the fixed `groundcontrol_app` role with the exact capabilities the design spec mandates (NOSUPERUSER/NOBYPASSRLS/NOCREATEDB/NOCREATEROLE/NOREPLICATION, CREATE only on `groundcontrol`, USAGE-only on `public`+`extensions`). Fresh init under the restricted role, object placement/ownership, migration version sanity, second-init idempotence, wrong-extension-placement fail-before-DDL, and cleanup that drops only the temp DB + role. Wired into `scripts/e2e-test-map.ts` for the dedicated-mode code paths.
