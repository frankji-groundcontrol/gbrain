...
| finding any other doc | `docs/README.md` — the documentation map |
...
- **Keep the doc records current.** `docs/README.md` maps the whole tree. After
  a commit-sized task: update `docs/architecture/` if structure or workflows
  changed, add reusable lessons to `docs/learning/`, drive multi-step work from
  a dated plan record in `docs/plans/`, and record reusable setups in
...
— that bundle the search cost/quality knobs (semantic cache, token budget, LLM
expansion, relational retrieval, result limit) into the single `search.mode`
config key, resolved through `src/core/search/mode.ts`:

    per-call SearchOpts → per-key config (search.cache.enabled, …) →
      MODE_BUNDLES[search.mode] → MODE_BUNDLES.balanced (fallback)

Paired with the downstream model, corner-to-corner cost spread is 25x; the
picker in `gbrain init` shows the matrix. Two invariants to preserve:

- Mode resolution lives in **bare `hybridSearch`** (NOT just the cached
  wrapper) so `gbrain eval replay` / `eval longmemeval` exercise the same
  mode-affected behavior as the production `query` op.
- Every result-shaping knob must fold into the `query_cache` `knobs_hash`
  (bump `mode.ts:KNOBS_HASH_VERSION`, the single source of truth) so one
  configuration's cached rows are never served to another.

Knob tables, cost anchors, relational retrieval, and the CLI surfaces:
`docs/operations/search-modes.md`. The cost matrix lives in that doc + the
`gbrain init` picker copy + `docs/eval/SEARCH_MODE_METHODOLOGY.md` — update all
three in lockstep (`test/init-mode-picker.test.ts` pins the anchor cells).
...
Read the skill files in `skills/` before doing brain operations. GBrain ships
~52 skills (the authoritative inventory is `skills/manifest.json`), organized
by `skills/RESOLVER.md` (`AGENTS.md` is also accepted as the router filename).
The groups below are orientation highlights, not the full inventory:
...
`minion-orchestrator` is the single unified skill for both lanes of background
work (shell jobs via `gbrain jobs submit shell`, LLM subagents via
`gbrain agent run`).
...
**Routing-table compression:** `skills/functional-area-resolver/` — two-layer
dispatch pattern for shrinking large AGENTS.md / RESOLVER.md files (>=12KB)
without losing routing accuracy: one entry per functional area, each declaring
its sub-skills in a `(dispatcher for: ...)` clause. The clause is the
load-bearing signal — stripping it collapses routing accuracy. Empirically
validated across three model tiers (+13 to +17pp over the verbose baseline at
48% the size). The A/B eval surface, committed receipts, and reproduction
commands live at `evals/functional-area-resolver/README.md` (outside `skills/`
deliberately so the skillpack bundler doesn't ship eval infrastructure).
...
`src/core/progress.ts`; agents get heartbeats within 1 second of every
iteration. Invariants: progress always writes to **stderr** (stdout stays clean
for `--json` data output); JSON events require the explicit `--progress-json`
flag; phase names are machine-stable `snake_case.dot.path`, additive changes
only; never write `\r` progress to stdout — `scripts/check-progress-to-stdout.sh`
(wired into `bun run test`) fails the build. The event schema and the
wiring-a-new-bulk-command recipe (createProgress / start / tick / finish /
startHeartbeat, Minion `job.updateProgress`): `docs/progress-events.md`.
...
`gbrain sync` is resumable and converges under pool exhaustion + repeated
kills: progress banks into the append-only `op_checkpoint_paths` table, a
killed run resumes from the checkpoint, `last_commit` only advances on true
completion, and the per-source lock refuses to steal a live, recently-refreshed
holder. Six `GBRAIN_SYNC_*` / `GBRAIN_LOCK_*` env knobs tune it (env-only
incident escape hatches by design — no config-dashboard surface). Knob table
and semantics: `docs/operations/sync-tuning.md`.

## Pace Mode (DB-contention-aware backfill pacing)

A naive `gbrain embed --stale` / large `sync` can saturate a PgBouncer
transaction-mode pooler and starve minion lock renewals (`lock-renewal-failed`
→ dead jobs). Pacing (`src/core/db-pacer.ts`, bundles in
`src/core/pace-mode.ts`) is the native fix — opt-in, default `off`, resolved
`per-call flag → GBRAIN_PACE_* env → config (pace.*) → PACE_BUNDLES[mode] → off`
(env ABOVE config as the incident escape hatch, unlike search modes).
Fail-open: a pacer bug never kills a backfill. Knob bundles, surfaces, and the
correctness fixes coupled to pacing: `docs/operations/pace-mode.md`.
