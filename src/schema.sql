...
CREATE OR REPLACE FUNCTION bump_page_generation_fn() RETURNS trigger SET search_path FROM CURRENT AS $func$
BEGIN
...
CREATE OR REPLACE FUNCTION bump_page_generation_clock_fn() RETURNS trigger SET search_path FROM CURRENT AS $func$
BEGIN
...
CREATE OR REPLACE FUNCTION update_chunk_search_vector() RETURNS TRIGGER SET search_path FROM CURRENT AS $fn$
BEGIN
  NEW.search_vector :=
...
-- + pglite-schema.ts.
-- search_path is pinned FROM CURRENT rather than a literal `pg_catalog,
-- public`: the body references timeline_entries unqualified, which lives in
-- `groundcontrol` under dedicated-schema mode. FROM CURRENT freezes the path
-- at CREATE time, closing the same caller-controlled-resolution hole.
CREATE OR REPLACE FUNCTION update_page_search_vector() RETURNS trigger SET search_path FROM CURRENT AS $$
DECLARE
  timeline_text TEXT;
BEGIN
...
CREATE OR REPLACE FUNCTION notify_minion_job_change() RETURNS trigger SET search_path FROM CURRENT AS $$
BEGIN
